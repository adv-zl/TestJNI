...
public class Main {
	...
	public static void main(String[] args) throws Exception {
        ...
        System.out.println(Encrypt(t2));
        System.out.println(DEncrypt(t));
        System.in.read();
	}


    public static String Encrypt(String os){
        try {
            byte[] gc = compress(os);
            byte[] dbs = TTEncryptUtils.e(gc,gc.length);

            return Base64.getEncoder().encodeToString(dbs);
        }catch (Exception ex){
            ex.printStackTrace();
        }
        return "";
    ...
    public static String DEncrypt(String os){
        try {
            byte[] bs = Base64.getDecoder().decode(os);
